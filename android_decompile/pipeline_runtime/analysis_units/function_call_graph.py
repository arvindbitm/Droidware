...
def set_high_priority():
    """Attempt to raise process priority when the host OS supports it."""
    try:
        p = psutil.Process(os.getpid())
        if hasattr(psutil, "HIGH_PRIORITY_CLASS"):
            p.nice(psutil.HIGH_PRIORITY_CLASS)
    except Exception as e:
        logger.warning("set_high_priority_failed", {"error": str(e)})
    ...
    try:
        ...
        logger.warning("set_priority_class_failed", {"error": str(e)})
...
# Ensure dataset-aligned tokens are tracked (including common aliases).
methods_to_track = list(
    dict.fromkeys(
        methods_to_track
        + [
            "ReadUserDictionary",
...
def extract_methods_and_calls(dx):
    ...
    try:
        ...
        # Deterministic single-thread processing avoids race conditions in shared
        # defaultdict/list counters and gives stable, repeatable feature counts.
