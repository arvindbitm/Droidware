...
model_path = os.path.join(BASE_DIR, "best_hybrid_model_state.pth")
model_versions_path = os.path.join(BASE_DIR, "model_versions")
log_file = os.path.join(BASE_DIR, "server_logs.log")
test_dataset_path = os.path.join(BASE_DIR, "Droidware2025_Set2.csv")
model_performance_log = os.path.join(BASE_DIR, "model_performance.csv")
update_validation_log = os.path.join(BASE_DIR, "update_validation_log.csv")
client_log_csv = os.path.join(BASE_DIR, "client_log.csv")
config_hash_path = os.path.join(BASE_DIR, "best_hybrid_model_config_hash.json")
blacklist_file = os.path.join(BASE_DIR, "blacklist.json")
master_log_file = os.path.join(BASE_DIR, "master_log.log")

os.makedirs(client_models_dir := os.path.join(BASE_DIR, "client_models"), exist_ok=True)
os.makedirs(model_versions_path, exist_ok=True)

global_model_folder = os.path.join(BASE_DIR, "global_model")
...
class FederatedLearningServer:
    ...
    @staticmethod
    def _optimizer_has_state(optimizer_state):
        return isinstance(optimizer_state, dict) and bool(optimizer_state.get("state"))

    @staticmethod
    def _clone_optimizer_state(optimizer_state):
        return copy.deepcopy(optimizer_state) if optimizer_state is not None else None

    def _build_checkpoint(self):
        if not self._optimizer_has_state(self.optimizer_state):
            recovered_state = None
            if self._optimizer_has_state(self.pre_update_optimizer_state):
                recovered_state = self._clone_optimizer_state(self.pre_update_optimizer_state)
            else:
                recovered_state = self._restore_optimizer_state_from_compatible_checkpoint(global_model_file)
            if recovered_state is not None:
                self.optimizer_state = recovered_state
        checkpoint = copy.deepcopy(getattr(self, "checkpoint_extras", {}))
        checkpoint["model_state_dict"] = self.model.state_dict()
        checkpoint["optimizer_state_dict"] = self._clone_optimizer_state(self.optimizer_state)
        checkpoint["config"] = copy.deepcopy(self.model_config)
        return checkpoint

    def _restore_optimizer_state_from_compatible_checkpoint(self, loaded_from):
        candidate_paths = []
        if os.path.exists(model_path):
            candidate_paths.append(model_path)
        if os.path.isdir(model_versions_path):
            candidate_paths.extend(
                os.path.join(model_versions_path, name)
                for name in sorted(os.listdir(model_versions_path), reverse=True)
                if name.endswith((".pth", ".pt"))
            )

        loaded_from_abs = os.path.abspath(loaded_from)
        current_param_count = len(list(self.model.parameters())) if self.model is not None else None

        for candidate_path in candidate_paths:
            if os.path.abspath(candidate_path) == loaded_from_abs or not os.path.exists(candidate_path):
                continue
            try:
                candidate_checkpoint = torch.load(candidate_path, map_location="cpu")
                if not isinstance(candidate_checkpoint, dict):
                    continue
                candidate_optimizer_state = candidate_checkpoint.get("optimizer_state_dict")
                if not self._optimizer_has_state(candidate_optimizer_state):
                    continue
                param_groups = candidate_optimizer_state.get("param_groups", [])
                if current_param_count is not None and param_groups:
                    flat_params = []
                    for group in param_groups:
                        flat_params.extend(group.get("params", []))
                    if len(flat_params) != current_param_count:
                        continue
                server_logger.warning(
                    f"Recovered optimizer state from '{candidate_path}' because '{loaded_from}' had an empty optimizer state."
                )
                return self._clone_optimizer_state(candidate_optimizer_state)
            except Exception as exc:
                server_logger.warning(f"Failed to inspect optimizer fallback '{candidate_path}': {exc}")

        return None

    def _aggregate_optimizer_states(self, client_optimizer_states, weights, previous_weight, device):
        weighted_states = []
        if self._optimizer_has_state(self.pre_update_optimizer_state):
            weighted_states.append((self.pre_update_optimizer_state, previous_weight))
        for state, weight in zip(client_optimizer_states, weights):
            if self._optimizer_has_state(state):
                weighted_states.append((state, weight))

        if not weighted_states:
            return self._clone_optimizer_state(self.pre_update_optimizer_state)

        optimizer = optim.AdamW(self.model.parameters(), lr=self.lr, weight_decay=0.01)
        aggregated_optimizer_state = optimizer.state_dict()

        reference_state = weighted_states[0][0]
        reference_param_groups = reference_state.get("param_groups", [])
        if reference_param_groups:
            for idx, ref_group in enumerate(reference_param_groups):
                if idx >= len(aggregated_optimizer_state["param_groups"]):
                    break
                for key, value in ref_group.items():
                    if key != "params":
                        aggregated_optimizer_state["param_groups"][idx][key] = copy.deepcopy(value)

        valid_param_ids = []
        for group in aggregated_optimizer_state.get("param_groups", []):
            valid_param_ids.extend(group.get("params", []))

        aggregated_optimizer_state["state"] = {}
        for param_id in valid_param_ids:
            param_entries = [(state["state"].get(param_id), weight) for state, weight in weighted_states if param_id in state.get("state", {})]
            if not param_entries:
                continue

            merged_state = {}
            reference_entry = param_entries[0][0]
            for key, reference_value in reference_entry.items():
                if torch.is_tensor(reference_value):
                    weighted_sum = None
                    contributing_weight = 0.0
                    for entry, weight in param_entries:
                        tensor_value = entry.get(key)
                        if not torch.is_tensor(tensor_value):
                            continue
                        tensor_value = tensor_value.detach().to(device=device, dtype=reference_value.dtype)
                        weighted_sum = tensor_value * weight if weighted_sum is None else weighted_sum + (tensor_value * weight)
                        contributing_weight += weight
                    if weighted_sum is not None and contributing_weight > 0:
                        merged_state[key] = (weighted_sum / contributing_weight).cpu()
                elif key == "step":
                    merged_state[key] = max(entry.get(key, 0) for entry, _ in param_entries)
                else:
                    merged_state[key] = copy.deepcopy(reference_value)

            if merged_state:
                aggregated_optimizer_state["state"][param_id] = merged_state

        if self._optimizer_has_state(aggregated_optimizer_state):
            return aggregated_optimizer_state
        return self._clone_optimizer_state(self.pre_update_optimizer_state)
            ...
            if isinstance(checkpoint, dict):
                if "model_state_dict" in checkpoint and "config" in checkpoint:
                    ...
                    if "optimizer_state_dict" in checkpoint:
                        ...
                        if not self._optimizer_has_state(self.optimizer_state):
                            recovered_state = self._restore_optimizer_state_from_compatible_checkpoint(file_path)
                            if recovered_state is not None:
                                self.optimizer_state = recovered_state
    ...
    async def aggregate_and_evaluate(self):
        ...
        async with model_lock:
            ...
            # Load aggregated states into the global model first
                ...
                aggregated_optimizer_state = self._aggregate_optimizer_states(
